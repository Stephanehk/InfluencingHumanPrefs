...
def main():
    ...
    #"pr_det_synth_prefs_100NO_STATS_UI_", "regret_det_synth_prefs_100NO_STATS_UI_"
    
    for i, preference_model in enumerate(["regret", "pr"]):
        for _, preference_assum in enumerate(["regret", "pr"]):

            for _, condition in enumerate(["same_start_state", "different_start_state"]):
                near_opts = []
                for n in partition_sizes:
                
                    
                    avg_returns = np.load("data/results/testing_things/"+condition+"_stoch_100NO_STATS_UI_" + preference_model + "_"+preference_assum+"_logistc_lin_partitioned" + "_avg_returns_n_split=" + str(n) + ".npy")
                ...
                print (near_opts)
                if condition == "different_start_state":
                    label += ", diff. start state"
                    ax[i].plot(x_ticks, near_opts, color=color, label=label, linestyle="--")
                else:
                    label += ", same start state"
                    ax[i].plot(x_ticks, near_opts, color=color, label=label)

        if preference_model == "regret":
            title = "regret labeled prefs."
        elif preference_model == "pr":
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("same_start_state_test_stoch_prefs.png", dpi=300)
